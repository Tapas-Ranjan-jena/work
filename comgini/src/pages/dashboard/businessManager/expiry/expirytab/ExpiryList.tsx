...
export default function ExpiryList() {
  ...
  useEffect(() => {
    const fetchExpired = async () => {
      setLoading(true);
      try {
        const data = await registrationService.getExpiringRegistrations(365);
      ...
      <table className="table table-sm table-bordered table-hover align-middle mb-0">
        <thead className="bg-light">
          <tr>
            ...
            <th className="fw-semibold text-danger">Expiry date</th>
            <th style={{ width: 40 }}>
          ...
          {loading ? (
            ...
            paginatedItems.map((item, idx) => (
              <tr key={idx} className="table-danger">
                ...
                <td className="text-danger fw-bold">{item.expiry_date}</td>
