...
export default function SetupLeaveTypes() {
  const [leaveTypes] = useState([
    ...
    { 
      id: 3, 
      description: "In Case of, \n• Any college Related Issues, \n• Any Educational related leave", 
      status: "Active" 
    }
  ]);

  return (
    <div className="setup-leave-types text-start">
          ...
          <table className="table table-hover align-middle mb-0" style={{ fontSize: "13.5px" }}>
            <thead className="table-light">
              <tr className="text-nowrap">
                <th className="fw-semibold px-3 py-3" style={{ minWidth: "300px" }}>Description</th>
                <th className="fw-semibold px-3 text-center" style={{ minWidth: "120px" }}>Status</th>
              ...
              {leaveTypes.map((type) => (
                <tr key={type.id}>
                  ...
                  <td className="text-center px-3">
                    <span className="fw-bold text-dark" style={{ fontSize: "13px" }}>{type.status}</span>
                  </td>
                  <td className="text-center px-3">
                    <div className="d-flex gap-2 justify-content-center">
                        <button className="btn btn-sm btn-white border rounded-circle p-1" style={{ width: '32px', height: '32px' }}><i className="bi bi-pencil fs-6 text-secondary"></i></button>
                        <button className="btn btn-sm btn-white border rounded-circle p-1" style={{ width: '32px', height: '32px' }}><i className="bi bi-x fs-5 text-secondary"></i></button>
                    </div>
                  </td>
                </tr>
              ))}
