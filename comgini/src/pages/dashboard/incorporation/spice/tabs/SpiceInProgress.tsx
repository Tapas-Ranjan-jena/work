...
export default function SpiceInProgress() {
  ...
  useEffect(() => {
    const fetchForms = async () => {
      setLoading(true);
      try {
        const res = await formsService.getForms("SPICE", "in_progress", page, pagination.limit, search);
        if (res.success) {
          setForms(res.data);
          setPagination(res.pagination);
        }
      } catch (error) {
        console.error("Error fetching SPICE in_progress forms:", error);
      } finally {
        setLoading(false);
      }
    };
